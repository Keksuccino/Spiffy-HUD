...
import de.keksuccino.spiffyhud.FMAnimationUtils;
...
public class MixinLoadingOverlay {
    ...
    @Unique private static SpiffyOverlayScreen drippyOverlayScreen = new SpiffyOverlayScreen();
    ...
    private void afterRenderDrippy(GuiGraphics graphics, int mouseX, int mouseY, float partial, CallbackInfo info) {
        ...
        this.setElementsOpacityDrippy(SpiffyHud.getOptions().earlyFadeOutElements.getValue() ? this.cachedElementOpacityDrippy : this.cachedBackgroundOpacityDrippy);
    ...
    private boolean cancelScreenRenderingDrippy(Screen instance, GuiGraphics guiGraphics, int i, int j, float f) {
        return SpiffyHud.getOptions().fadeOutLoadingScreen.getValue();
    }
    ...
    private void cancelOriginalProgressBarRenderingDrippy(GuiGraphics graphics, int p_96184_, int p_96185_, int p_96186_, int p_96187_, float opacity, CallbackInfo info) {
        if (!this.shouldRenderVanillaDrippy()) {
            info.cancel();
            this.cachedElementOpacityDrippy = SpiffyHud.getOptions().fadeOutLoadingScreen.getValue() ? opacity : 1.0F;
    ...
    private boolean cancelBackgroundRenderingDrippy(GuiGraphics instance, RenderType renderType, int i, int j, int k, int l, int color) {
        this.cachedBackgroundOpacityDrippy = SpiffyHud.getOptions().fadeOutLoadingScreen.getValue() ? Math.min(1.0F, Math.max(0.0F, (float)FastColor.ARGB32.alpha(color) / 255.0F)) : 1.0F;
        return this.shouldRenderVanillaDrippy();
    }
    ...
    private void setNewOverlayScreenDrippy() {
        drippyOverlayScreen = new SpiffyOverlayScreen();
