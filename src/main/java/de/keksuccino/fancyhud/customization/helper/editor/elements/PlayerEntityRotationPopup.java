...
public class PlayerEntityRotationPopup extends FHPopup {
	...
	public PlayerEntityRotationPopup(LayoutEditorScreen handler, LayoutMirroredPlayer object) {
		...
		this.applyButton = new AdvancedButton(0, 0, 100, 20, Locals.localize("fancyhud.popup.apply"), true, (call) -> {
			this.updateValues();
		});
	...
	public void render(MatrixStack matrix, int mouseX, int mouseY, Screen renderIn) {
		...
		drawCenteredString(matrix, font, Locals.localize("fancyhud.helper.creator.items.mirroredplayer.rotation.custom.bodyx"), midX, midY - 83, Color.WHITE.getRGB());
		
		this.bodyX.x = midX - (this.bodyX.getWidth() / 2);
		this.bodyX.y = midY - 70;
		this.bodyX.render(matrix, mouseX, mouseY, partial);
		
		drawCenteredString(matrix, font, Locals.localize("fancyhud.helper.creator.items.mirroredplayer.rotation.custom.bodyy"), midX, midY - 43, Color.WHITE.getRGB());
		
		this.bodyY.x = midX - (this.bodyY.getWidth() / 2);
		this.bodyY.y = midY - 30;
		this.bodyY.render(matrix, mouseX, mouseY, partial);

		drawCenteredString(matrix, font, Locals.localize("fancyhud.helper.creator.items.mirroredplayer.rotation.custom.headx"), midX, midY - 3, Color.WHITE.getRGB());
		
		this.headX.x = midX - (this.headX.getWidth() / 2);
		this.headX.y = midY + 10;
		this.headX.render(matrix, mouseX, mouseY, partial);

		drawCenteredString(matrix, font, Locals.localize("fancyhud.helper.creator.items.mirroredplayer.rotation.custom.heady"), midX, midY + 37, Color.WHITE.getRGB());
