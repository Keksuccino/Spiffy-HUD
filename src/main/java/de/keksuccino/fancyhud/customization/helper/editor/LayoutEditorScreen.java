...
import de.keksuccino.fancyhud.customization.helper.editor.elements.vanilla.CrosshairLayoutElement;
import de.keksuccino.fancyhud.customization.helper.editor.elements.vanilla.ExperienceJumpBarLayoutElement;
import de.keksuccino.fancyhud.customization.helper.editor.elements.vanilla.FoodMountHealthLayoutElement;
...
import de.keksuccino.fancyhud.customization.helper.ui.popup.FHNotificationPopup;
...
//		this.content.add(this.potionIconsLayoutElement);
	...
	protected void renderVanillaElements(MatrixStack matrix, float partial) {
		...
		this.ingameHud.renderIngameGui(matrix, partial);
...
//	}
	...
	protected void addTexture(String path) {
		...
		if (f.exists()) {
			if (filename.equals(f.getName())) {
				...
				displayNotification(Locals.localize("fancyhud.helper.creator.textures.invalidcharacters"), "", "", "", "", "", "");
			}
		} else {
			displayNotification("§c§l" + Locals.localize("fancyhud.helper.creator.invalidimage.title"), "", Locals.localize("fancyhud.helper.creator.invalidimage.desc"), "", "", "", "", "", "");
		}
	}
	...
	protected void addWebTexture(String url) {
		...
		if (WebUtils.isValidUrl(finalUrl)) {
			...
			displayNotification(Locals.localize("fancyhud.helper.creator.web.invalidurl"), "", "", "", "", "", "");
		}
	}
	...
	protected void addSlideshow(String name) {
		...
		if (SlideshowHandler.slideshowExists(name)) {
			...
			displayNotification(Locals.localize("fancyhud.helper.creator.slideshownotfound"), "", "", "", "");
		}
	}
	...
	protected void addMultiSplashText(String path) {
		...
		if (f.exists() && f.getPath().toLowerCase().endsWith(".txt")) {
			...
			displayNotification(Locals.localize("fancyhud.helper.creator.error.invalidfile"));
		}
	}
	...
	protected void addSingleSplashText(String content) {
		...
		if (!content.replace(" ", "").equals("")) {
			...
			displayNotification("§c§l" + Locals.localize("fancyhud.helper.creator.texttooshort.title"), "", Locals.localize("fancyhud.helper.creator.texttooshort.desc"), "", "", "", "");
		}
	}
...
//	}
		...
		if (!l.isEmpty()) {
			if (l.size() == 1) {
				if (l.get(0).isDestroyable()) {
					...
					displayNotification("§c§l" + Locals.localize("fancyhud.helper.creator.cannotdelete.title"), "", Locals.localize("fancyhud.helper.creator.cannotdelete.desc"), "", "", "");
				}
			}
			if (l.size() > 1) {
				if (FancyHud.config.getOrDefault("editordeleteconfirmation", true)) {
					...
					}, "§c§l" + Locals.localize("fancyhud.helper.creator.messages.sure"), "", Locals.localize("fancyhud.helper.creator.deleteselectedobjects"), "", "", "", "", ""));
				} else {
					this.deleteContentQueue.addAll(l);
				}
			}
	...
	public void saveLayoutAs() {
		PopupHandler.displayPopup(new FHTextInputPopup(new Color(0, 0, 0, 0), Locals.localize("fancyhud.helper.editor.ui.layout.saveas.entername"), null, 240, (call) -> {
			try {

				if ((call != null) && (call.length() > 0)) {

					String file = FancyHud.CUSTOMIZATION_DIR.getPath() + "/" + call + ".fhlayout";
					File f = new File(file);

					if (!f.exists()) {
						if (!CustomizationHandler.saveLayoutTo(this.getAllProperties(), file)) {
							PopupHandler.displayPopup(new FHNotificationPopup(300, new Color(0, 0, 0, 0), 240, null, Locals.localize("fancyhud.helper.editor.ui.layout.saveas.failed")));
						} else {
							Snapshot snap = this.history.createSnapshot();
