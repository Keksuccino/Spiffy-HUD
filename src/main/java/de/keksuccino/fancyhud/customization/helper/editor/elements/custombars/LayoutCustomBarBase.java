...
import de.keksuccino.fancyhud.customization.items.custombars.CustomBarCustomizationItemBase;
...
public class LayoutCustomBarBase extends LayoutElement {
	...
	public void init() {
		...
		/** BAR COLOR **/
		AdvancedButton barColorButton = new AdvancedButton(0, 0, 0, 16, Locals.localize("fancyhud.helper.creator.add.custombars.barcolor"), true, (press) -> {
			FHTextInputPopup pop = new FHTextInputPopup(new Color(0, 0, 0, 0), Locals.localize("fancyhud.helper.creator.add.custombars.barcolor"), null, 240, (call) -> {
				if (call != null) {
		...
		/** BACKGROUND COLOR **/
		AdvancedButton backgroundColorButton = new AdvancedButton(0, 0, 0, 16, Locals.localize("fancyhud.helper.creator.add.custombars.backgroundcolor"), true, (press) -> {
			FHTextInputPopup pop = new FHTextInputPopup(new Color(0, 0, 0, 0), Locals.localize("fancyhud.helper.creator.add.custombars.backgroundcolor"), null, 240, (call) -> {
				if (call != null) {
		...
		/** BAR TEXTURE **/
		AdvancedButton barTextureButton = new AdvancedButton(0, 0, 0, 16, Locals.localize("fancyhud.helper.creator.add.custombars.bartexture"), true, (press) -> {
		...
		barTextureButton.setDescription(StringUtils.splitLines(Locals.localize("fancyhud.helper.creator.add.custombars.bartexture.btn.desc"), "%n%"));
		this.rightclickMenu.addContent(barTextureButton);
		
		/** BACKGROUND TEXTURE **/
		AdvancedButton backgroundTextureButton = new AdvancedButton(0, 0, 0, 16, Locals.localize("fancyhud.helper.creator.add.custombars.backgroundtexture"), true, (press) -> {
			ChooseFilePopup pop = new ChooseFilePopup((call) -> {
				if (call != null) {
		...
		backgroundTextureButton.setDescription(StringUtils.splitLines(Locals.localize("fancyhud.helper.creator.add.custombars.backgroundtexture.btn.desc"), "%n%"));
		this.rightclickMenu.addContent(backgroundTextureButton);
		
		this.rightclickMenu.addSeparator();
		
		/** DIRECTION **/
		String barDirString = Locals.localize("fancyhud.helper.creator.add.custombars.direction.left");
		BarDirection d = this.getObject().direction;
		if (d == BarDirection.RIGHT) {
			barDirString = Locals.localize("fancyhud.helper.creator.add.custombars.direction.right");
		}
		if (d == BarDirection.UP) {
			barDirString = Locals.localize("fancyhud.helper.creator.add.custombars.direction.up");
		}
		if (d == BarDirection.DOWN) {
			barDirString = Locals.localize("fancyhud.helper.creator.add.custombars.direction.down");
		}
		AdvancedButton barDirectionButton = new AdvancedButton(0, 0, 0, 16, barDirString, true, (press) -> {
			...
			if (bd == BarDirection.LEFT) {
				...
				b.setMessage(Locals.localize("fancyhud.helper.creator.add.custombars.direction.right"));
			} else if (bd == BarDirection.RIGHT) {
				this.getObject().direction = BarDirection.UP;
				b.setMessage(Locals.localize("fancyhud.helper.creator.add.custombars.direction.up"));
			} else if (bd == BarDirection.UP) {
				this.getObject().direction = BarDirection.DOWN;
				b.setMessage(Locals.localize("fancyhud.helper.creator.add.custombars.direction.down"));
			} else if (bd == BarDirection.DOWN) {
				this.getObject().direction = BarDirection.LEFT;
				b.setMessage(Locals.localize("fancyhud.helper.creator.add.custombars.direction.left"));
			}
		});
