...
public class ExperienceJumpBarLayoutElement extends VanillaLayoutElement {
	...
	public void init() {
		...
		/** LEVEL VISIBILITY **/
		String showLvlString = Locals.localize("fancyhud.helper.editor.elements.vanilla.exp.level.visibility.on");
		if (!ob.showLvl) {
			showLvlString = Locals.localize("fancyhud.helper.editor.elements.vanilla.exp.level.visibility.off");
		}
		AdvancedButton showLvlButton = new AdvancedButton(0, 0, 0, 0, showLvlString, (press) -> {
			this.handler.history.saveSnapshot(this.handler.history.createSnapshot());
			if (ob.showLvl) {
				((AdvancedButton)press).setMessage(Locals.localize("fancyhud.helper.editor.elements.vanilla.exp.level.visibility.off"));
				ob.showLvl = false;
			} else {
				((AdvancedButton)press).setMessage(Locals.localize("fancyhud.helper.editor.elements.vanilla.exp.level.visibility.on"));
				ob.showLvl = true;
			}
		});
		...
		/** LEVEL COLOR **/
		AdvancedButton lvlColorButton = new AdvancedButton(0, 0, 0, 0, Locals.localize("fancyhud.helper.editor.elements.vanilla.exp.level.color"), (press) -> {
			FHTextInputPopup pop = new FHTextInputPopup(new Color(0, 0, 0, 0), Locals.localize("fancyhud.helper.editor.elements.vanilla.exp.level.color"), null, 240, (call) -> {
		...
		lvlColorButton.setDescription(StringUtils.splitLines(Locals.localize("fancyhud.helper.editor.elements.vanilla.exp.level.color.desc", ""), "%n%"));
