...
public class LayoutItemStack extends LayoutElement {
    ...
    public void init() {
        ...
        AdvancedButton setBySlotButton = new AdvancedButton(0, 0, 0, 16, Locals.localize("fancyhud.helper.creator.items.itemstack.setitem.byslot"), true, (press) -> {
            FHTextInputPopup pop = new FHTextInputPopup(new Color(0, 0, 0, 0), Locals.localize("fancyhud.helper.creator.items.itemstack.setitem.byslot"), CharacterFilter.getIntegerCharacterFiler(), 240, (call) -> {
                if (call != null) {
                    if (call.replace(" ", "").equals("")) {
        ...
        setBySlotButton.setDescription(StringUtils.splitLines(Locals.localize("fancyhud.helper.creator.items.itemstack.setitem.byslot.btn.desc"), "%n%"));
        setItemMenu.addContent(setBySlotButton);

        AdvancedButton setByIdButton = new AdvancedButton(0, 0, 0, 16, Locals.localize("fancyhud.helper.creator.items.itemstack.setitem.byid"), true, (press) -> {
            FHTextInputPopup pop = new FHTextInputPopup(new Color(0, 0, 0, 0), Locals.localize("fancyhud.helper.creator.items.itemstack.setitem.byid"), null, 240, (call) -> {
                if (call != null) {
                    if (call.replace(" ", "").equals("")) {
                        if (this.getObject().itemId != null) {
                            this.handler.history.saveSnapshot(this.handler.history.createSnapshot());
        ...
        setByIdButton.setDescription(StringUtils.splitLines(Locals.localize("fancyhud.helper.creator.items.itemstack.setitem.byid.btn.desc"), "%n%"));
        setItemMenu.addContent(setByIdButton);

        AdvancedButton setItemButton = new AdvancedButton(0, 0, 0, 16, Locals.localize("fancyhud.helper.creator.items.itemstack.setitem"), true, (press) -> {
        ...
        setItemButton.setDescription(StringUtils.splitLines(Locals.localize("fancyhud.helper.creator.items.itemstack.setitem.btn.desc"), "%n%"));
        this.rightclickMenu.addContent(setItemButton);

        String stackSizeLabel = Locals.localize("fancyhud.helper.creator.items.itemstack.showstacksize.on");
        if (!this.getObject().renderStackSize) {
            stackSizeLabel = Locals.localize("fancyhud.helper.creator.items.itemstack.showstacksize.off");
        }
        AdvancedButton showStackSizeButton = new AdvancedButton(0, 0, 0, 16, stackSizeLabel, true, (press) -> {
            if (this.getObject().renderStackSize) {
                ((AdvancedButton)press).setMessage(Locals.localize("fancyhud.helper.creator.items.itemstack.showstacksize.off"));
                this.getObject().renderStackSize = false;
            } else {
                ((AdvancedButton)press).setMessage(Locals.localize("fancyhud.helper.creator.items.itemstack.showstacksize.on"));
                this.getObject().renderStackSize = true;
            }
        });
        this.rightclickMenu.addContent(showStackSizeButton);

        String durabLabel = Locals.localize("fancyhud.helper.creator.items.itemstack.showdurability.on");
        if (!this.getObject().renderDurability) {
            durabLabel = Locals.localize("fancyhud.helper.creator.items.itemstack.showdurability.off");
        }
        AdvancedButton showDurabilityButton = new AdvancedButton(0, 0, 0, 16, durabLabel, true, (press) -> {
            if (this.getObject().renderDurability) {
                ((AdvancedButton)press).setMessage(Locals.localize("fancyhud.helper.creator.items.itemstack.showdurability.off"));
                this.getObject().renderDurability = false;
            } else {
                ((AdvancedButton)press).setMessage(Locals.localize("fancyhud.helper.creator.items.itemstack.showdurability.on"));
