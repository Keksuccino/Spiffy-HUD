...
public class LayoutSplashText extends LayoutElement {
	...
	public void init() {
		...
		/** SCALE **/
		AdvancedButton scaleButton = new AdvancedButton(0, 0, 0, 0, Locals.localize("fancyhud.helper.creator.items.string.setscale"), true, (press) -> {
			FHTextInputPopup p = new FHTextInputPopup(new Color(0, 0, 0, 0), "§l" + Locals.localize("fancyhud.helper.creator.items.string.setscale") + ":", CharacterFilter.getDoubleCharacterFiler(), 240, this::setScaleCallback);
		...
		/** ROTATION **/
		AdvancedButton rotationButton = new AdvancedButton(0, 0, 0, 0, Locals.localize("fancyhud.helper.creator.items.splash.rotation"), true, (press) -> {
			FHTextInputPopup p = new FHTextInputPopup(new Color(0, 0, 0, 0), "§l" + Locals.localize("fancyhud.helper.creator.items.splash.rotation") + ":", CharacterFilter.getDoubleCharacterFiler(), 240, (call) -> {
				if (call != null) {
		...
		/** BASE COLOR **/
		AdvancedButton colorButton = new AdvancedButton(0, 0, 0, 16, Locals.localize("fancyhud.helper.creator.items.splash.basecolor"), true, (press) -> {
			FHTextInputPopup t = new FHTextInputPopup(new Color(0, 0, 0, 0), "§l" + Locals.localize("fancyhud.helper.creator.items.splash.basecolor") + ":", null, 240, (call) -> {
				if (call != null) {
					if (!call.equals("")) {
		...
		/** SHADOW **/
		String shadowLabel = Locals.localize("fancyhud.helper.creator.items.string.setshadow");
		if (this.getObject().shadow) {
			shadowLabel = Locals.localize("fancyhud.helper.creator.items.string.setnoshadow");
		}
		...
		/** BOUNCING **/
		String bounceLabel = Locals.localize("fancyhud.helper.creator.items.splash.bounce.off");
		if (this.getObject().bounce) {
			bounceLabel = Locals.localize("fancyhud.helper.creator.items.splash.bounce.on");
		}
		AdvancedButton bounceButton = new AdvancedButton(0, 0, 0, 0, bounceLabel, true, (press) -> {
			if (this.getObject().bounce) {
				((AdvancedButton)press).setMessage(Locals.localize("fancyhud.helper.creator.items.splash.bounce.off"));
				this.handler.history.saveSnapshot(this.handler.history.createSnapshot());
				
				this.getObject().bounce = false;
			} else {
				((AdvancedButton)press).setMessage(Locals.localize("fancyhud.helper.creator.items.splash.bounce.on"));
				this.handler.history.saveSnapshot(this.handler.history.createSnapshot());
		...
		String refreshLabel = Locals.localize("fancyhud.helper.creator.items.splash.refresh.off");
		if (this.getObject().refreshOnMenuReload) {
			refreshLabel = Locals.localize("fancyhud.helper.creator.items.splash.refresh.on");
		}
		AdvancedButton refreshButton = new AdvancedButton(0, 0, 0, 0, refreshLabel, true, (press) -> {
			if (this.getObject().refreshOnMenuReload) {
				((AdvancedButton)press).setMessage(Locals.localize("fancyhud.helper.creator.items.splash.refresh.off"));
				this.handler.history.saveSnapshot(this.handler.history.createSnapshot());
				
				this.getObject().refreshOnMenuReload = false;
			} else {
				((AdvancedButton)press).setMessage(Locals.localize("fancyhud.helper.creator.items.splash.refresh.on"));
				this.handler.history.saveSnapshot(this.handler.history.createSnapshot());
				
				this.getObject().refreshOnMenuReload = true;
			}
		});
		refreshButton.setDescription(StringUtils.splitLines(Locals.localize("fancyhud.helper.creator.items.splash.refresh.desc"), "%n%"));
