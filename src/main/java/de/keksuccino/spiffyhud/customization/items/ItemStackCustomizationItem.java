...
public class ItemStackCustomizationItem extends CustomizationItemBase {
    ...
    public void render(PoseStack matrix) {
        ...
        if (this.slot != -1000) {
            ...
            this.value = "Item Stack: " + this.item.getHoverName().getString();
        }
        ...
        if (this.shouldRender()) {
            ...
            if (!isEditorActive()) {
                if (this.item != null) {
                    ItemRenderUtils.renderGuiItem(this.getPosX() + 2, this.getPosY() + 2, Minecraft.getInstance().getFrameTime(), this.item, this.scale, this.renderStackSize, this.renderDurability);
                }
            } else {
                if ((this.itemId != null) && ((this.item == null) || (this.item.getItem() instanceof AirItem))) {
                    ...
                    drawCenteredString(matrix, Minecraft.getInstance().font, "Slot", scaledX, scaledY, -1);
                    drawCenteredString(matrix, Minecraft.getInstance().font, "" + this.slot, scaledX, scaledY2, -1);
                    matrix.popPose();
                } else if (this.item != null) {
                    ItemRenderUtils.renderGuiItem(this.getPosX() + 2, this.getPosY() + 2, Minecraft.getInstance().getFrameTime(), this.item, this.scale, this.renderStackSize, this.renderDurability);
                }
        ...
        if (p != null) {
            if (p.getInventory() != null) {
                ...
                if (this.slot == -1) {
                    ...
                    stack = p.getInventory().getItem(this.slot);
                }
