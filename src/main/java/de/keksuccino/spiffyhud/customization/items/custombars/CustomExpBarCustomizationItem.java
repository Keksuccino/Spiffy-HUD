...
public class CustomExpBarCustomizationItem extends CustomBarCustomizationItemBase {
	
	protected int currentPercentWidthHeight = 0;

	public CustomExpBarCustomizationItem(PropertiesSection item) {
		super(item);
		
		this.value = "Custom EXP Bar";
	...
	public void render(PoseStack matrix) {
		...
		if (p != null) {
			...
			float expPercent = p.experienceProgress * 100.0F;
			if (this.isEditor()) {
				expPercent = 50.0F;
			}
			if ((this.direction == BarDirection.LEFT) || (this.direction == BarDirection.RIGHT)) {
				this.currentPercentWidthHeight = (int)((((float)this.width) / 100.0F) * expPercent);
			}
			if ((this.direction == BarDirection.UP) || (this.direction == BarDirection.DOWN)) {
				this.currentPercentWidthHeight = (int)((((float)this.height) / 100.0F) * expPercent);
			}
