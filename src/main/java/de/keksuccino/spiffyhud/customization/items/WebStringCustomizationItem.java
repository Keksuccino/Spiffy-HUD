	...
	public void render(MatrixStack matrix) {
		...
		if (!this.updating) {
			...
			for (Map.Entry<String, Float> m : this.lines.entrySet()) {
				...
				if (this.shadow) {
					font.drawStringWithShadow(matrix, "§f" + line, x, y + (i / sc), 0 | MathHelper.ceil(this.opacity * 255.0F) << 24);
				} else {
					font.drawString(matrix, "§f" + line, x, y + (i / sc), 0 | MathHelper.ceil(this.opacity * 255.0F) << 24);
				}
				matrix.pop();
