...
public class WebStringCustomizationItem extends CustomizationItemBase {
	...
	public volatile Map<String, Float> lines = new LinkedHashMap<>();
	...
	public void render(MatrixStack matrix) {
		...
		if (!this.updating) {
			...
			for (Map.Entry<String, Float> m : this.lines.entrySet()) {
				...
				int x = (int) (this.getPosX() / sc);
				int y = (int) (this.getPosY() / sc);
				...
				if (isEditorActive()) {
					...
					line = DynamicValueHelper.convertFromRaw(m.getKey());
				}
				
				matrix.push();
				matrix.scale(sc, sc, sc);
				if (this.shadow) {
					font.drawWithShadow(matrix, "§f" + line, x, y + (i / sc), 0 | MathHelper.ceil(this.opacity * 255.0F) << 24);
				} else {
					font.draw(matrix, "§f" + line, x, y + (i / sc), 0 | MathHelper.ceil(this.opacity * 255.0F) << 24);
				}
				matrix.pop();
		...
		new Thread(() -> {
			...
			if (isValidUrl()) {
				...
				try {
					...
					lines.clear();
					String s = Locals.localize("spiffyhud.customization.items.webstring.unabletoload");
					lines.put(s, 1.0F);
