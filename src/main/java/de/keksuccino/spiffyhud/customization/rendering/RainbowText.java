...
public class RainbowText extends GuiComponent {
	...
	public void render(PoseStack matrix, int x, int y) {
		if (this.text != null) {
			...
			Component c = TextComponentUtils.getWithGradient(start, this.text, end);
			if (this.shadow) {
				font.drawShadow(matrix, c.getVisualOrderText(), x, y, -1);
			} else {
				font.draw(matrix, c.getVisualOrderText(), x, y, -1);
			}
