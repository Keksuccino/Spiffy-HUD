...
public class LayoutString extends LayoutElement {
	...
	public void init() {
		...
		AdvancedButton scaleB = new AdvancedButton(0, 0, 0, 16, Locals.localize("spiffyhud.helper.creator.items.string.setscale"), true, (press) -> {
			PopupHandler.displayPopup(new FHTextInputPopup(new Color(0, 0, 0, 0), "§l" + Locals.localize("spiffyhud.helper.creator.items.string.setscale") + ":", CharacterFilter.getDoubleCharacterFiler(), 240, this::setScaleCallback));
		});
		this.rightclickMenu.addContent(scaleB);
		
		String sLabel = Locals.localize("spiffyhud.helper.creator.items.string.setshadow");
		if (this.getObject().shadow) {
			sLabel = Locals.localize("spiffyhud.helper.creator.items.string.setnoshadow");
		}
		AdvancedButton shadowB = new AdvancedButton(0, 0, 0, 16, sLabel, true, (press) -> {
			if (this.getObject().shadow) {
				((AdvancedButton)press).setMessage(Locals.localize("spiffyhud.helper.creator.items.string.setshadow"));
				this.handler.history.saveSnapshot(this.handler.history.createSnapshot());
				
				this.getObject().shadow = false;
			} else {
				((AdvancedButton)press).setMessage(Locals.localize("spiffyhud.helper.creator.items.string.setnoshadow"));
				this.handler.history.saveSnapshot(this.handler.history.createSnapshot());
...
//
//		String al = Locals.localize("spiffyhud.helper.creator.items.string.alignment.left");
...
//			((AdvancedButton)press).setMessage("§a" + Locals.localize("spiffyhud.helper.creator.items.string.alignment.left"));
//			alignmentRightBtn.setMessage(Locals.localize("spiffyhud.helper.creator.items.string.alignment.right"));
//			alignmentCenteredBtn.setMessage(Locals.localize("spiffyhud.helper.creator.items.string.alignment.centered"));
//		});
//		alignmentMenu.addContent(alignmentLeftBtn);
//
//		String ar = Locals.localize("spiffyhud.helper.creator.items.string.alignment.right");
...
//			((AdvancedButton)press).setMessage("§a" + Locals.localize("spiffyhud.helper.creator.items.string.alignment.right"));
//			alignmentLeftBtn.setMessage(Locals.localize("spiffyhud.helper.creator.items.string.alignment.left"));
//			alignmentCenteredBtn.setMessage(Locals.localize("spiffyhud.helper.creator.items.string.alignment.centered"));
//		});
//		alignmentMenu.addContent(alignmentRightBtn);
//
//		String ac = Locals.localize("spiffyhud.helper.creator.items.string.alignment.centered");
...
//			((AdvancedButton)press).setMessage("§a" + Locals.localize("spiffyhud.helper.creator.items.string.alignment.centered"));
//			alignmentRightBtn.setMessage(Locals.localize("spiffyhud.helper.creator.items.string.alignment.right"));
//			alignmentLeftBtn.setMessage(Locals.localize("spiffyhud.helper.creator.items.string.alignment.left"));
//		});
//		alignmentMenu.addContent(alignmentCenteredBtn);
//
//		AdvancedButton alignmentBtn = new AdvancedButton(0, 0, 0, 0, Locals.localize("spiffyhud.helper.creator.items.string.alignment"), true, (press) -> {
//			alignmentMenu.setParentButton((AdvancedButton) press);
//			alignmentMenu.openMenuAt(0, press.y);
//		});
//		alignmentBtn.setDescription(StringUtils.splitLines(Locals.localize("spiffyhud.helper.creator.items.string.alignment.desc"), "%n%"));
//		this.rightclickMenu.addContent(alignmentBtn);
		
		AdvancedButton editTextB = new AdvancedButton(0, 0, 0, 16, Locals.localize("spiffyhud.helper.creator.items.string.edit"), true, (press) -> {
			DynamicValueInputPopup i = new DynamicValueInputPopup(new Color(0, 0, 0, 0), "§l" + Locals.localize("spiffyhud.helper.creator.items.string.edit") + ":", null, 240, this::setTextCallback);
		...
		/** RAINBOW MODE **/
		String rainbowToggleString = Locals.localize("spiffyhud.helper.editor.elements.string.rainbow.on");
		if (!this.getObject().rainbowMode) {
			rainbowToggleString = Locals.localize("spiffyhud.helper.editor.elements.string.rainbow.off");
		}
		AdvancedButton rainbowToggleButton = new AdvancedButton(0, 0, 0, 16, rainbowToggleString, true, (press) -> {
			if (this.getObject().rainbowMode) {
				((AdvancedButton)press).setMessage(Locals.localize("spiffyhud.helper.editor.elements.string.rainbow.off"));
				this.getObject().rainbowMode = false;
			} else {
				((AdvancedButton)press).setMessage(Locals.localize("spiffyhud.helper.editor.elements.string.rainbow.on"));
				this.getObject().rainbowMode = true;
			}
		});
		rainbowToggleButton.setDescription(StringUtils.splitLines(Locals.localize("spiffyhud.helper.editor.elements.string.rainbow.btn.desc"), "%n%"));
		this.rightclickMenu.addContent(rainbowToggleButton);
		
		AdvancedButton rainbowColorButton = new AdvancedButton(0, 0, 0, 16, Locals.localize("spiffyhud.helper.editor.elements.string.rainbow.colors"), true, (press) -> {
		...
		AdvancedButton rainbowFadeSpeedButton = new AdvancedButton(0, 0, 0, 16, Locals.localize("spiffyhud.helper.editor.elements.string.rainbow.speed"), true, (press) -> {
			FHTextInputPopup pop = new FHTextInputPopup(new Color(0, 0, 0, 0), Locals.localize("spiffyhud.helper.editor.elements.string.rainbow.speed"), CharacterFilter.getDoubleCharacterFiler(), 240, (call) -> {
				if (call != null) {
					float newSpeed = 1.0F;
	...
	protected void renderBorder(MatrixStack matrix, int mouseX, int mouseY) {
		...
		font.drawString(matrix, Locals.localize("spiffyhud.helper.creator.items.border.orientation")+ ": " + this.object.orientation, this.getStringPosX()*2, (this.getStringPosY()*2) - 44, Color.WHITE.getRGB());
		font.drawString(matrix, Locals.localize("spiffyhud.helper.creator.items.string.border.scale") + ": " + this.getStringScale(), this.getStringPosX()*2, (this.getStringPosY()*2) - 35, Color.WHITE.getRGB());
		font.drawString(matrix, Locals.localize("spiffyhud.helper.creator.items.string.border.alignment") + ": " + this.getObject().alignment.key, this.getStringPosX()*2, (this.getStringPosY()*2) - 26, Color.WHITE.getRGB());
		font.drawString(matrix, Locals.localize("spiffyhud.helper.creator.items.border.posx") + ": " + this.getStringPosX(), this.getStringPosX()*2, (this.getStringPosY()*2) - 17, Color.WHITE.getRGB());
		font.drawString(matrix, Locals.localize("spiffyhud.helper.creator.items.border.width") + ": " + this.object.width, this.getStringPosX()*2, (this.getStringPosY()*2) - 8, Color.WHITE.getRGB());
		font.drawString(matrix, Locals.localize("spiffyhud.helper.creator.items.border.posy") + ": " + this.getStringPosY(), ((this.getStringPosX() + this.object.width)*2)+3, ((this.getStringPosY() + this.object.height)*2) - 14, Color.WHITE.getRGB());
		font.drawString(matrix, Locals.localize("spiffyhud.helper.creator.items.border.height") + ": " + this.object.height, ((this.getStringPosX() + this.object.width)*2)+3, ((this.getStringPosY() + this.object.height)*2) - 5, Color.WHITE.getRGB());
...
//		this.setHeight((int)(7*scale));
	...
	private void setTextCallback(String text) {
		...
		if (text.length() > 0) {
			...
			LayoutEditorScreen.displayNotification("§c§l" + Locals.localize("spiffyhud.helper.creator.texttooshort.title"), "", Locals.localize("spiffyhud.helper.creator.texttooshort.desc"), "", "", "", "");
		}
	}
	...
	private void setScaleCallback(String scale) {
		...
		if (MathUtils.isFloat(scale)) {
			...
			LayoutEditorScreen.displayNotification("§c§l" + Locals.localize("spiffyhud.helper.creator.items.string.scale.invalidvalue.title"), "", Locals.localize("spiffyhud.helper.creator.items.string.scale.invalidvalue.desc"), "", "", "", "", "");
		}
	}
