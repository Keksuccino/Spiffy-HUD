...
import de.keksuccino.spiffyhud.customization.items.CustomizationItemBase.Alignment;
...
public class LayoutWebString extends LayoutElement {
	...
	public void init() {
		...
		String mLabel = Locals.localize("spiffyhud.helper.creator.webstring.multiline");
		if (this.getObject().multiline) {
			mLabel = Locals.localize("spiffyhud.helper.creator.webstring.nomultiline");
		}
		AdvancedButton multilineB = new AdvancedButton(0, 0, 0, 16, mLabel, true, (press) -> {
			if (this.getObject().multiline) {
				((AdvancedButton)press).setMessage(Locals.localize("spiffyhud.helper.creator.webstring.multiline"));
				this.handler.history.saveSnapshot(this.handler.history.createSnapshot());
				
				this.getObject().multiline = false;
				this.getObject().updateContent(this.getObject().value);
			} else {
				((AdvancedButton)press).setMessage(Locals.localize("spiffyhud.helper.creator.webstring.nomultiline"));
				this.handler.history.saveSnapshot(this.handler.history.createSnapshot());
