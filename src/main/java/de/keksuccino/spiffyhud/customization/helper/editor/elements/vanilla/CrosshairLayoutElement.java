...
public class CrosshairLayoutElement extends VanillaLayoutElement {
	...
	public void init() {
		...
		/** CROSSHAIR TEXTURE **/
		AdvancedButton textureButton = new AdvancedButton(0, 0, 0, 0, Locals.localize("spiffyhud.helper.editor.elements.vanilla.crosshair.texture"), (press) -> {
		...
		textureButton.setDescription(StringUtils.splitLines(Locals.localize("spiffyhud.helper.editor.elements.vanilla.crosshair.texture.desc", ""), "%n%"));
		this.rightclickMenu.addContent(textureButton);
		
		/** INVERT COLORS **/
		String invertString = Locals.localize("spiffyhud.helper.editor.elements.vanilla.crosshair.invertcolors.on");
		if (!ob.invertCrosshairColors) {
			invertString = Locals.localize("spiffyhud.helper.editor.elements.vanilla.crosshair.invertcolors.off");
		}
		AdvancedButton invertButton = new AdvancedButton(0, 0, 0, 0, invertString, (press) -> {
			if (ob.invertCrosshairColors) {
				ob.invertCrosshairColors = false;
				((AdvancedButton)press).setMessage(Locals.localize("spiffyhud.helper.editor.elements.vanilla.crosshair.invertcolors.off"));
			} else {
				ob.invertCrosshairColors = true;
				((AdvancedButton)press).setMessage(Locals.localize("spiffyhud.helper.editor.elements.vanilla.crosshair.invertcolors.on"));
			}
		});
		invertButton.setDescription(StringUtils.splitLines(Locals.localize("spiffyhud.helper.editor.elements.vanilla.crosshair.invertcolors.desc"), "%n%"));
		this.rightclickMenu.addContent(invertButton);
		
		/** ATTACK INDICATOR **/
		String attackIndicString = Locals.localize("spiffyhud.helper.editor.elements.vanilla.crosshair.attackindicator.on");
		if (!ob.renderAttackIndicator) {
			attackIndicString = Locals.localize("spiffyhud.helper.editor.elements.vanilla.crosshair.attackindicator.off");
		}
		AdvancedButton attackIndicatorButton = new AdvancedButton(0, 0, 0, 0, attackIndicString, (press) -> {
			if (ob.renderAttackIndicator) {
				ob.renderAttackIndicator = false;
				((AdvancedButton)press).setMessage(Locals.localize("spiffyhud.helper.editor.elements.vanilla.crosshair.attackindicator.off"));
			} else {
				ob.renderAttackIndicator = true;
				((AdvancedButton)press).setMessage(Locals.localize("spiffyhud.helper.editor.elements.vanilla.crosshair.attackindicator.on"));
			}
		});
		attackIndicatorButton.setDescription(StringUtils.splitLines(Locals.localize("spiffyhud.helper.editor.elements.vanilla.crosshair.attackindicator.desc"), "%n%"));
