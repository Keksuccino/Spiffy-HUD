...
public class OverlayMessageLayoutElement extends VanillaLayoutElement {
	...
	public void init() {
		...
		/** BASE COLOR **/
		AdvancedButton lvlColorButton = new AdvancedButton(0, 0, 0, 0, Locals.localize("spiffyhud.helper.editor.elements.vanilla.overlaymessage.color"), (press) -> {
			FHTextInputPopup pop = new FHTextInputPopup(new Color(0, 0, 0, 0), Locals.localize("spiffyhud.helper.editor.elements.vanilla.overlaymessage.color"), null, 240, (call) -> {
				if (call != ob.colorHex) {
		...
		lvlColorButton.setDescription(StringUtils.splitLines(Locals.localize("spiffyhud.helper.editor.elements.vanilla.overlaymessage.color.desc", ""), "%n%"));
