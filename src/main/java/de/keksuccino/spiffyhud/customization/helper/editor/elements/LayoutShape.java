...
public class LayoutShape extends LayoutElement {
	...
	public void init() {
		...
		AdvancedButton colorB = new AdvancedButton(0, 0, 0, 16, Locals.localize("spiffyhud.helper.creator.items.shape.color"), true, (press) -> {

			FHTextInputPopup t = new FHTextInputPopup(new Color(0, 0, 0, 0), "§l" + Locals.localize("spiffyhud.helper.creator.items.shape.color") + ":", null, 240, (call) -> {
				if (call != null) {
					if (!call.equals("")) {
		...
		colorB.setDescription(StringUtils.splitLines(Locals.localize("spiffyhud.helper.creator.items.shape.color.btndesc"), "%n%"));
