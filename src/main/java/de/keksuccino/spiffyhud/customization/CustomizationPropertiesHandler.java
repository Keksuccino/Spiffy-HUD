...
public class CustomizationPropertiesHandler {
	...
	private static List<PropertiesSet> disabled = new ArrayList<>();

	public static void loadProperties() {
		File f = SpiffyHud.CUSTOMIZATION_DIR;
		properties = parsePropertiesFromDir(f);

		File f3 = new File(SpiffyHud.CUSTOMIZATION_DIR.getPath() + "/.disabled");
	...
	public static List<PropertiesSet> parsePropertiesFromDir(File dir) {
		...
		for (File f2 : dir.listFiles()) {
			if (f2.getPath().toLowerCase().endsWith(".fhlayout")) {
				...
				if ((s != null) && s.getPropertiesType().equalsIgnoreCase("hud")) {
					...
					if (!l.isEmpty()) {
						...
						if (!isVersionCompatible(s4, s5, SpiffyHud.VERSION)) {
							continue;
						}
